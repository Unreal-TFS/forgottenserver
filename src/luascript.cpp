...
BestiaryInfo LuaScriptInterface::getBestiaryInfo(lua_State* L, int32_t arg)
{
	...
	uint16_t prowess = getField<uint16_t>(L, arg, "firstUnlock");
	uint16_t expertise = getField<uint16_t>(L, arg, "secondUnlock");
	uint16_t mastery = getField<uint16_t>(L, arg, "finishUnlock");
	...
	lua_pop(L, 9);
	return BestiaryInfo(className, raceId, prowess, expertise, mastery, charmPoints, stars, occurrence, locations);
}
...
void LuaScriptInterface::pushBestiaryInfo(lua_State* L, const BestiaryInfo& info)
{
	...
	setField(L, "firstUnlock", info.prowess);
	setField(L, "secondUnlock", info.expertise);
	setField(L, "finishUnlock", info.mastery);
...
{
	// Game.getBestiary()
	auto& bestiary = g_monsters.bestiary;
	lua_createtable(L, 0, 3);
	lua_pushnumber(L, bestiary->getMonsterCount());
	lua_setfield(L, -2, "monstersCount");

	const auto& classes = bestiary->getBestiary();
	size_t classesCount = classes.size();
	lua_pushnumber(L, classesCount);
	lua_setfield(L, -2, "classesCount");

	lua_createtable(L, 0, classesCount);
	for (const auto& [className, monsters] : classes) {
		lua_createtable(L, monsters.size(), 0);
		int index = 0;
		for (const auto& it : monsters) {
			pushUserdata<const MonsterType>(L, g_monsters.getMonsterType(it.second));
			setMetatable(L, -1, "MonsterType");
			lua_rawseti(L, -2, ++index);
		}
		lua_setfield(L, -2, className.c_str());
	}
...
int LuaScriptInterface::luaMonsterTypeBestiaryInfo(lua_State* L)
{
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (g_monsters.bestiary->isValid(info)) {
				g_monsters.bestiary->addMonsterType(info.className, info.raceId, monsterType->name);

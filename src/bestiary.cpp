...
void Bestiary::addMonsterType(const std::string& className, uint16_t raceId, std::string_view monsterName)
{
	if (bestiary.contains(className) && bestiary.at(className).contains(raceId)) {
		std::cout << "[Warning - Bestiary::addMonsterrType] raceId " << raceId << " already exists for class "
		          << className << std::endl;
		return;
	}
...
bool Bestiary::isValid(const BestiaryInfo& info) const
{
	...
	if (info.prowess >= info.expertise || info.expertise >= info.mastery) {
		std::cout
		    << "[Warning - Bestiary::isValid] prowess must be lower than expertise and expertise must be lower than mastery."
		    << std::endl;
		return false;
	}

	if (info.stars > BESTIARY_MAX_STARS) {
		std::cout << "[Warning - Bestiary::isValid] stars can't be higher than " << BESTIARY_MAX_STARS << '.'
		          << std::endl;
		return false;
	}

	if (info.occurrence > BESTIARY_MAX_OCCURRENCE) {
		std::cout << "[Warning - Bestiary::isValid] occurrence can't be higher than " << BESTIARY_MAX_OCCURRENCE << '.'
		          << std::endl;
		return false;
	}
